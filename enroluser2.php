...
http://nen.moodle.com.au/test/auth/rsa/enroluser.php?username=rsa115&firstname=Renal115&email=teststudent115@example.com&timestamp=1454028812&hash=454119e7ee8b04617d62e21ff9f2b610&lastname=Society&course=test101
...
//        $out .= html_writer::div('User ID: '.$userid);
    ...
    //        $out .= html_writer::div('Course ID: '.$course->id);
            ...
            if (!($ue = $DB->get_record('user_enrolments', array('userid' => $userid, 'enrolid' => $enrol->id)))) {
                    ...
                    $jsonoutput['enrolment'] = ($ueid = $DB->insert_record('user_enrolments', $userenrolment)) ? 'You are enrolled in '.$course->fullname : 'You were not enrolled in a course';
              ...
              //  $out .= html_writer::div(($ueid = $DB->insert_record('user_enrolments', $userenrolment)) ? 'You are enrolled in '.$course->fullname : 'You were not enrolled in a course');
                    ...
                    $jsonoutput['enrolment'] = 'You are enrolled in '.$course->fullname;
...
//print_object($from);
            ...
            $emailtext->url                 = $CFG->wwwroot."/course/index.php?id=".$course->id;
...
//                $out .= html_writer::div("Admin NOT emailed enrolment details for ".$user->email);
    ...
    $jsonoutput['message'] = get_string('db:nocourse', 'auth_rsa');
        ...
        if ($userid and $ueid and $raid)
        {
            ...
            $cpdlog->timestarted    = time();
            $cpdlog->timefinished   = 0;
            $cpdlog->timemodified   = time();
    ...
    $jsonoutput['status'] = 'failed';
           //      throw new coding_exception(get_string('db:insert:error:cpdlog', 'auth_rsa'));
            ...
            $jsonoutput['enrolment'] = 'User not enrolled';
